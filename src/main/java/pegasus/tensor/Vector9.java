...
/**
 * An immutable nine-dimensional vector.
 *
 * @see Tensor
 * @see Vector
 * @see Vector2
...
public class Vector9 extends ArrayVector<Vector9> {
     ...
     */
    public Vector9(double i, double j, double k, double l, double m, double n, double o, double p, double q) {
     ...
     * @throws IllegalArgumentException When the provided vector {@code v}'s size is not {@code 9}
     * @throws NullPointerException     When the provided vector {@code v} is {@code null}
     */
    public Vector9(Vector<?> v) {
        super(v.toArray());

        if (v.size() != 9) {
            throw new IllegalArgumentException("The provided vector's size is not 9.");
        }
    }
     ...
     */
    @Override
    public Vector9 map(DoubleUnaryOperator mapper) {
        return new Vector9(stream().map(mapper).toArray());
    }
     ...
     */
    @Override
    public Vector9 merge(Vector9 v, DoubleBinaryOperator merger) {
